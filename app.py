...
selected_page = st.sidebar.radio("Select a page:", ["About", "Methodology", "Solution", "Documentation", "Disclaimer"])
...
def load_protected_page(page_name):
    ...
    module_path = os.path.join(os.path.dirname(__file__), "protected_pages", f"{page_name}.py")
    
    # Check if the file exists before attempting to load it
    if not os.path.exists(module_path):
        st.error(f"The requested page '{page_name}' does not exist. Path: {module_path}")
        return
    ...
    try:
        ...
        if spec and spec.loader:
            ...
            if hasattr(page_module, "app"):
                ...
                st.error(f"The page '{page_name}' does not have an 'app()' function.")
        else:
            st.error(f"Failed to load the page '{page_name}'. Module specification issue.")
    except Exception as e:
        st.error(f"An error occurred while loading the page '{page_name}': {str(e)}")

# Display the selected page
if selected_page == "About":
    load_protected_page("about")
elif selected_page == "Methodology":
    load_protected_page("methodology")
    ...
    load_protected_page("documentation")
elif selected_page == "Disclaimer":
    load_protected_page("disclaimer")